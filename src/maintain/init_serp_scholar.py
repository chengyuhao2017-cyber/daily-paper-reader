...
def main() -> None:
    parser = argparse.ArgumentParser(description="抓取 SerpAPI Google Scholar 论文并同步到 Supabase。")
    ...
    if raw_path:
        ...
        raw_path = os.path.join(project_root, "archive", date_str, "raw", f"serp_scholar_papers_{date_str}.json")

    if not args.skip_fetch:
        fetch_cmd = [
            python,
            os.path.join(SCRIPT_DIR, "fetchers", "fetch_serp_scholar.py"),
            "--output",
            raw_path,
            "--num-per-query",
        ...
        year_from = int(args.year_from or 0)
        if year_from:
            fetch_cmd += ["--year-from", str(year_from)]
        if args.ignore_seen:
            fetch_cmd.append("--ignore-seen")
        run_step("Step 1 - fetch SerpAPI Scholar", fetch_cmd)
    else:
    ...
    run_step("Step 2 - sync SerpAPI Scholar to Supabase", sync_cmd)
