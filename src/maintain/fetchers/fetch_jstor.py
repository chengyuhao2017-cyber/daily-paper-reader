...
通过 JSTOR Data for Research API 检索学术文献。
"""
...
CRAWL_STATE_FILE = os.path.join(ROOT_DIR, "archive", "jstor_crawl_state.json")
SEEN_IDS_FILE = os.path.join(ROOT_DIR, "archive", "jstor_seen.json")

API_BASE = "https://www.jstor.org/api/search"
SOURCE_KEY = "jstor"
...
def fetch_papers(query: str, days: int = 7, max_results: int = 50) -> List[Dict[str, Any]]:
    ...
    headers = {"User-Agent": "DailyPaperReader/1.0", "Accept": "application/json"}
    params = {
        "Query": query, "pageSize": min(max_results, 25), "page": 1,
    ...
    for attempt in range(3):
        try:
            ...
            log(f"[WARN] JSTOR API 请求失败 (第{attempt + 1}次)：{exc}")
            if attempt < 2:
    ...
    for item in results:
        if not isinstance(item, dict):
            continue
        stable_url = _norm(item.get("href") or item.get("url") or "")
        item_id = _norm(item.get("id") or item.get("iid") or "")
        if not item_id and stable_url:
            item_id = stable_url.rstrip("/").split("/")[-1]
        if not item_id:
            continue
        ...
        if isinstance(authors_raw, str):
            ...
            authors = [_norm(a) if isinstance(a, str) else _norm(a.get("name", "")) for a in authors_raw]
        else:
            authors = []
        pub_date = _norm(item.get("publishedDate") or item.get("py") or "")
        url = stable_url or f"https://www.jstor.org/stable/{item_id}"
        papers.append({
            "id": f"jstor:{item_id}", "title": _norm(item.get("title") or ""),
            "abstract": _norm(item.get("abstract") or item.get("description") or ""),
            "authors": authors, "published": pub_date, "updated_at": pub_date,
            "url": url, "pdf_url": "",
            "categories": [_norm(item.get("discipline") or "economics")], "source": SOURCE_KEY,
        })
    log(f"JSTOR 检索到 {len(papers)} 篇论文 (query={query!r})")
    return papers


def main() -> None:
    parser = argparse.ArgumentParser(description="JSTOR 论文抓取")
