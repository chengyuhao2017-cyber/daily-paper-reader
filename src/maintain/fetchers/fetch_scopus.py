...
def _make_paper_id(doi: str, eid: str, title: str) -> str:
    if doi:
        clean = re.sub(r"[^a-zA-Z0-9/_.:@-]", "", doi)
        return f"doi:{clean}"
    if eid:
        return f"scopus:{eid}"
    fp = f"{title.lower().strip()}|scopus"
...
def extract_search_queries(config: Dict[str, Any]) -> List[Dict[str, str]]:
    ...
    for profile in profiles or []:
        ...
        for kw_entry in (profile.get("keywords") or []):
            ...
            if kw_text:
                queries.append({"query": kw_text, "tag": tag, "profile": tag})
        ...
        for iq in (profile.get("intent_queries") or []):
            ...
            if q:
                queries.append({"query": q, "tag": tag, "profile": tag})
...
def _normalize_scopus_entry(
    ...
    eid = _norm(entry.get("eid") or "")

    # Link: prefer scopus inward link
    links = entry.get("link") or []
    link = ""
    if isinstance(links, list):
        for l in links:
            if isinstance(l, dict) and l.get("@ref") == "scopus":
    ...
    # Abstract: not available in standard view; try dc:description
    abstract = _norm(entry.get("dc:description") or "")

    # Authors from dc:creator (format: "LastName I., LastName2 I.")
    authors: List[str] = []
    creator = _norm(entry.get("dc:creator") or "")
    if creator:
        authors = [a.strip() for a in re.split(r",\s*|;\s*", creator) if a.strip()]

    # Publication date from prism:coverDate (format: "2026-12-14")
    published_str = ""
    ...
    paper_id = _make_paper_id(doi, eid, title)
...
def fetch_scopus(
    ...
    for i, q_info in enumerate(queries[:total_queries], start=1):
        query = q_info["query"]
        tag = q_info.get("tag", "")
        group_start(f"Scopus query {i}/{total_queries}: {query[:60]}")
        log(f"   [{i}/{total_queries}] tag={tag!r} | query={query!r}")

        raw_data = _search_scopus(
            query=query,
            api_key=api_key,
            count=num_per_query,
        )
        ...
        log(f"   Got {len(entries)} entries from Scopus (total results: {total_results}).")
        new_in_query = 0

        for entry in entries:
            if not isinstance(entry, dict):
                continue
            paper = _normalize_scopus_entry(entry, tag)
            if not paper:
                continue
            paper_id = paper["id"]
    ...
    log(f"[INFO] Scopus fetch complete: {len(papers)} unique papers collected.")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Search Elsevier Scopus using config.yaml keywords."
    )
    parser.add_argument(
        "--api-key", type=str,
    ...
    if not output_path:
        ...
        output_path = os.path.join(raw_dir, f"scopus_papers_{token}.json")
    ...
    log(f"[INFO] Running {len(queries)} queries via Elsevier Scopus Search API.")
