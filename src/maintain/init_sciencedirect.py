...
def main() -> None:
    parser = argparse.ArgumentParser(description="抓取 Elsevier ScienceDirect 论文并同步到 Supabase。")
    ...
    if raw_path:
        ...
        raw_path = os.path.join(project_root, "archive", date_str, "raw", f"sciencedirect_papers_{date_str}.json")

    if not args.skip_fetch:
        fetch_cmd = [
            python,
            os.path.join(SCRIPT_DIR, "fetchers", "fetch_sciencedirect.py"),
            "--output",
            raw_path,
            "--num-per-query",
        ...
        run_step("Step 1 - fetch ScienceDirect", fetch_cmd)
    else:
    ...
    run_step("Step 2 - sync ScienceDirect to Supabase", sync_cmd)
