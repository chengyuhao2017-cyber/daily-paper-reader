...
def main() -> None:
    parser = argparse.ArgumentParser(description="维护入口：Elsevier ScienceDirect 抓取 + Supabase 同步。")
    ...
    cleanup_backend(backend_key="sciencedirect", retention_days=args.retention_days, skip_cleanup=args.skip_cleanup)

    raw_path = str(args.raw_input or "").strip() or default_raw_path("sciencedirect_papers", run_date)
    ...
    init_cmd = [
        sys.executable,
        os.path.join(os.path.dirname(__file__), "init_sciencedirect.py"),
        "--num-per-query",
    ...
    run_step("Maintain ScienceDirect", init_cmd)
